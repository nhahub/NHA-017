...
def main():
    spark = SparkSession.builder.appName("Transform_Fazwaz").getOrCreate()

    df = spark.read.parquet("hdfs://namenode:9000/datalake/silver/cleaned_fazwaz")

    if "name" in df.columns: df = df.withColumnRenamed("name", "title")
    ...
    df = df.withColumn("source", lit("FazWaz"))
    ...
    output_path = "hdfs://namenode:9000/datalake/gold/fazwaz_gold"
    df.write.mode("overwrite").parquet(output_path)
    print(f"✅ FazWaz Gold data (Enriched) saved to {output_path}")
